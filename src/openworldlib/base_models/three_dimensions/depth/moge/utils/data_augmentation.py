...
def sample_perspective(
    ...
    raw_fov_x, raw_fov_y = intrinsics_to_fov(src_intrinsics)
    ...
    tgt_fov_x_min = min(fov_range_relative_min * raw_fov_x, focal_to_fov(fov_to_focal(fov_range_relative_min * raw_fov_y) / tgt_aspect))
    tgt_fov_x_max = min(fov_range_relative_max * raw_fov_x, focal_to_fov(fov_to_focal(fov_range_relative_max * raw_fov_y) / tgt_aspect))
    ...
    tgt_fov_y = focal_to_fov(fov_to_focal(tgt_fov_x) * tgt_aspect)
    ...
    direction = unproject_cv(np.array([[cu, cv]], dtype=np.float32), np.array([1.0], dtype=np.float32), intrinsics=src_intrinsics)[0]
    
    # 3. obtain the rotation matrix for homography warping (new_ext = R * old_ext)
    R = rotation_matrix_from_vectors(direction, np.array([0, 0, 1], dtype=np.float32))
    ...
    for i in range(4):
        intersection, _ = ray_intersection(
    ...
    tgt_intrinsics = intrinsics_from_focal_center(fx, fy, 0.5, 0.5).astype(np.float32)
...
def warp_perspective(
    ...
    if interpolation == 'lanczos' and (scale_x < 0.8 or scale_y < 0.8):
        ...
        src_map, _ = masked_nearest_resize(src_map, mask=sparse_mask, size=(src_height, src_width))
