...
from . import build, fifo_server, platform, wsgi_server

from ._quality_settings import DEFAULT_QUALITY, QUALITY_SETTINGS
from .exceptions import UnityCrashException
from .hooks.metadata_hook import MetadataHook
from .interact import DefaultActions, InteractiveControllerPrompt
from .platform import STR_PLATFORM_MAP
from .server import DepthFormat, MetadataWrapper
from .util import atomic_write, makedirs
from .util.lock import LockEx
...
class Controller(object):
        ...
        **unity_initialization_parameters,
        ...
        if self.server_class is None:
            ...
            if server_class is None and platform_system() == "Windows":
                self.server_class = wsgi_server.WsgiServer
            elif (
                isinstance(server_class, fifo_server.FifoServer)
                and platform_system() == "Windows"
            ):
                raise ValueError("server_class=FifoServer cannot be used on Windows.")
            elif server_class is None:
                self.server_class = fifo_server.FifoServer
            else:
        ...
        if not start_unity:
            self._build = build.EditorBuild()
        elif local_executable_path:
            self._build = build.ExternalBuild(local_executable_path)
        else:
        ...
        if not download_only:
            ...
            if (
                ...
                and self.server_class != fifo_server.FifoServer
            ):
    ...
    def _build_server(self, host, port, width, height):
        ...
        if self.server_class == wsgi_server.WsgiServer:
            self.server = wsgi_server.WsgiServer(
                host=host,
        ...
        elif self.server_class == fifo_server.FifoServer:
            # Not supported on Windows
            if os.name == "nt":
                raise ValueError("FIFO server not supported on Windows platform.")
            self.server = fifo_server.FifoServer(
                width=width,
                height=height,
                timeout=self.server_timeout,
                depth_format=self.depth_format,
    ...
    def reset(self, scene=None, **init_params):
        ...
        if self._build.platform == platform.CloudRendering:
    ...
    def server_type_to_class(cls, server_type):
        if server_type == fifo_server.FifoServer.server_type:
            return fifo_server.FifoServer
        elif server_type == wsgi_server.WsgiServer.server_type:
            return wsgi_server.WsgiServer
        else:
            valid_servers = str.join(
                [
                    f"'{x}'"
                    for x in [
                        fifo_server.FifoServer.server_type,
                        wsgi_server.WsgiServer.server_type,
                    ]
                ],
                ",",
            )
    ...
    def next_interact_command(self):
        ...
        from .interact import get_term_character
    ...
    def find_build(self, local_build, commit_id, branch, platform):
        ...
        if platform is None:
            candidate_platforms = platform.select_platforms(request)
        else:
            candidate_platforms = [
        ...
        for build in builds:
            if build.platform.is_valid(request):
                ...
                if (
                    ...
                    and build.platform != platform.CloudRendering
                ):
                    warnings.warn(
