...
# Adapted from lingbot-va/wan_va/modules/model.py for OpenWorldLib integration.
import math
...
class WanAttention(torch.nn.Module):
    ...
    def __init__(
        ...
        if attn_mode == 'torch':
            ...
            if flash_attn_func is None:
                raise ImportError("flash_attn is required for attn_mode='flashattn'")
    ...
    def forward(self, q, k, v, rotary_emb, update_cache=0, cache_name='pos'):
        kv_cache = self.attn_caches[cache_name] if self.attn_caches is not None else None
...
class WanTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, ffn_dim, num_heads, cross_attn_norm=False, eps=1e-6, attn_mode: str = "flashattn"):
        super().__init__()
        self.attn_mode = attn_mode

        self.norm1 = FP32LayerNorm(dim, eps, elementwise_affine=False)
        self.attn1 = WanAttention(dim=dim, heads=num_heads, dim_head=dim // num_heads, eps=eps,
                                  cross_attention_dim_head=None, attn_mode=attn_mode)
        self.attn2 = WanAttention(dim=dim, heads=num_heads, dim_head=dim // num_heads, eps=eps,
                                  cross_attention_dim_head=dim // num_heads, attn_mode=attn_mode)
    ...
    def forward(self, hidden_states, encoder_hidden_states, temb, rotary_emb,
                update_cache=0, cache_name='pos') -> torch.Tensor:
...
class WanTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    @register_to_config
    def __init__(self, patch_size=[1, 2, 2], num_attention_heads=24, attention_head_dim=128,
                 in_channels=48, out_channels=48, action_dim=30, text_dim=4096, freq_dim=256,
                 ffn_dim=14336, num_layers=30, cross_attn_norm=True, eps=1e-06,
                 rope_max_seq_len=1024, pos_embed_seq_len=None, attn_mode="torch"):
        super().__init__()
        self.patch_size = patch_size
    ...
    def create_empty_cache(self, cache_name, attn_window, latent_token_per_chunk,
                           action_token_per_chunk, device, dtype, batch_size):
        total_tolen = (attn_window // 2) * latent_token_per_chunk + (attn_window // 2) * action_token_per_chunk
        for block in self.blocks:
            block.attn1.init_kv_cache(cache_name, total_tolen, self.num_attention_heads,
    ...
    def forward(self, input_dict, update_cache=0, cache_name="pos", action_mode=False):
        ...
        for block in self.blocks:
            latent_hidden_states = block(latent_hidden_states, text_hidden_states, timestep_proj,
                                         rotary_emb, update_cache=update_cache, cache_name=cache_name)
        ...
        latent_hidden_states = (self.norm_out(latent_hidden_states.float()) * (1. + scale) + shift).type_as(latent_hidden_states)

        if action_mode:
            latent_hidden_states = self.action_proj_out(latent_hidden_states)
        else:
            latent_hidden_states = self.proj_out(latent_hidden_states)
            latent_hidden_states = rearrange(latent_hidden_states, 'b l (n c) -> b (l n) c', n=math.prod(self.patch_size))
